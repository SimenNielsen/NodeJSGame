...
var healthBarRed = document.createElement('div');
var healthBarGreen = document.createElement('div');
healthBarGreen.className = "healthbarGreen";
var healthText = document.createElement('div');
healthText.innerHTML = "100";
healthText.className = "healthbarText";
healthBarRed.appendChild(healthBarGreen);
healthBarRed.className = "healthbarRed";
document.body.appendChild(renderer.domElement);
document.body.appendChild(healthBarRed);
...
const spherePrimitive = Primitives.createSphere(boxMaterial, 32);
...
let playerInfo = new playerStats();
...
playerCollisionObject.setOnIntersectListener((delta, entity) => {
    ...
    playerInfo.removeHealth();
    healthBarGreen.style.width = playerInfo.currentHealth + "%";
    healthText.innerHTML = playerInfo.currentHealth;
    if(playerInfo.checkLose()){
...
function loop(now) {
    ...
    if (move.mode === 0) {
        ...
        player.applyTranslation(velocity[0], velocity[1], velocity[2]); // using the spread operator ( equivalent to ..applyTranslation(velocity[0], velocity[1], velocity[2]); )
        player.translation[0] = Math.min(Math.max(player.translation[0], -3.75), 3.75);
